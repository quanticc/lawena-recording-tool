...
public interface Resources {
    ...
    /**
     * Scan a given directory for {@link Resource} elements. Only the direct children of <code>dir</code> will be
     * scanned as potential <code>Resource</code>s. Each element found will be processed using {@link
     * #newResource(Path)}.
     *
    ...
    /**
     * Attempt to create a new {@link Resource} for the given <code>path</code> or update it if already exists. It may
     * or may not return a result, indicating if the path pointed to an actual <code>Resource</code> or not.
     *
    ...
    /**
     * Attempt to remove a potential {@link Resource} existing under the given <code>path</code>. This method does not
     * remove any file in the filesystem.
     *
     * @param path the <code>Path</code> that contains the <code>Resource</code>
     * @return <code>true</code> if the <code>Resource</code> existed and could be deleted, <code>false</code> otherwise
     */
     ...
     *
     * @return a <code>ListProperty</code> of <code>Resource</code> objects currently stored in this repository.
     */
    ReadOnlyListProperty<Resource> resourceListProperty();
    ...
    List<String> getResourceContents(Resource resource);
}
